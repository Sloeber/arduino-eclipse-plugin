...
public class BuildBehaviourTab extends AbstractAutoBuildPropertyTab {
	 ...
	 *          N/A 5: N/A 6: N/A
	 */
	static int[] calc3states(ICPropertyProvider p, int mode) {
		if (p.isMultiCfg()) {
			boolean m0 = (mode == 0);
			boolean m1 = (mode == 1);
			...
			b[0] = m0 ? bldr0.isManagedBuildOn() : (m1 ? bldr0.stopOnFirstBuildError() : bldr0.isAutoBuildEnable());
			...
			b[4] = m0 ? bldr0.useDefaultBuildCommand() : false;
			b[5] = (m0 || m1) ? bldr0.useStandardBuildArguments() : false;
			b[6] = (m0 || m1) ? !bldr0.useStandardBuildArguments() : false;
	...
	protected void updateButtons() {
		IAutoBuildConfigurationDescription autoConfDesc = AutoBuildProject
				.getAutoBuildConfig(page.getCfgsEditable()[0]);
		canModify = false;
		...
		if (extStates == null) { // no extended states available
			setTriSelection(r_useStandardBuildArguments, autoConfDesc.useStandardBuildArguments());
			setTriSelection(r_useCustomBuildArguments, !autoConfDesc.useStandardBuildArguments());
		} else {
		...
		// Stop on error
		boolean defaultBuildArguments = autoConfDesc.useStandardBuildArguments();
		if (defaultBuildArguments) {
			if (extStates != null) {
				...
				setTriSelection(b_stopOnError, autoConfDesc.stopOnFirstBuildError());
				b_stopOnError
						.setEnabled(autoConfDesc.supportsStopOnError(true) && autoConfDesc.supportsStopOnError(false));
			}
		} else {
			b_stopOnError.setEnabled(defaultBuildArguments);
		}

		updateParallelBlock(autoConfDesc, defaultBuildArguments);
		...
		if (extStates != null) {
			...
			setTriSelection(b_autoBuild, autoConfDesc.isAutoBuildEnable());
			setTriSelection(b_cmdBuild, autoConfDesc.isIncrementalBuildEnabled());
			setTriSelection(b_cmdClean, autoConfDesc.isCleanBuildEnabled());
		}
...
//		}
	...
	private void updateParallelBlock(IAutoBuildConfigurationDescription autoConfDesc, boolean defaultBuildArguments) {
		// note: for multi-config selection bldr is from Active cfg

		boolean isParallelSupported = autoConfDesc.supportsParallelBuild();
		boolean isParallelOn = autoConfDesc.isParallelBuild();
		int triSelection = isParallelOn ? TRI_YES : TRI_NO;

		int parallelizationNumInternal = autoConfDesc.getParallelizationNum();
		int optimalParallelNumber = autoConfDesc.getOptimalParallelJobNum();
		int parallelNumber = autoConfDesc.getParallelizationNum();
		...
		if (needsUpdate) {
				...
				for (ICConfigurationDescription i : page.getCfgsEditable()) {
					...
					if (b == r_useStandardBuildArguments) {
						bld.setUseStandardBuildArguments(val);
						...
						bld.setStopOnFirstBuildError(val);
					} else if (b == b_parallel) {
						bld.setIsParallelBuild(val);
					}
				}
...
//		updateData(getResDesc());
	...
	private boolean isInternalBuilderEnabled() {
			for (ICConfigurationDescription cfg : page.getCfgsEditable()) {
				IAutoBuildConfigurationDescription b = AutoBuildProject.getAutoBuildConfig(cfg);
				return b.isInternalBuilderEnabled();
			}
		return false;
	}
