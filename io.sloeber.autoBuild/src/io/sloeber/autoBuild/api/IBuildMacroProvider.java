...
public interface IBuildMacroProvider {
     ...
     *            and Macro Suppliers in order to obtain the macro value. The type
     *            of the context data
     *            differs depending on the context type and can be one of the
     *            following:
     *            1. IFileContextData interface - used to represent currently
     ...
     *            IOption getOption();
     *            }
     *            NOTE: the IFileContextData is passed that represents the current
     *            file and the option
     *            for that file because Macro Value Provider needs to know what
     *            option should be used
     *            as a context in case macro is not found for "current file" context
     *            2. IOptionContextData interface used to represent the currently
     *            selected option context
     *            3. IConfiguration - used to represent the currently selected
    ...
    /**
     * This method is defined to be used primarily by the UI classes and should not
     * be used by the
     * tool-integrator
     * 
     ...
     *            expanded to. If null the BuildMacroException is thrown in case the
     *            string to be resolved
     *            references inexistent macros
     * @param listDelimiter
     *            if not null, StringList macros are expanded as
     *            "<value_1>< listDelimiter ><value_2>< listDelimiter > ...
     *            <value_n>"
     *            otherwise the BuildMacroException is thrown in case the string to
     *            be resolved references
     *            string-list macros
     * @param contextType
     *            context from which the macro search should be started
     * @param contextData
     *            context data
     */
     ...
     * 1. In case when a user has specified to resolve the environment build macros
     * all macros
     * get resolved in the string
     * 2. In case when the a user has specified not to resolve the environment build
     * macros all macros
     * get resolved except the build environment macros (macros whose name conflicts
     * with one
     * of reserved macro names, or string-list macros always get resolved in the
     * buildfile).
     * Macro references that are kept unresolved are converted to the makefile
     * format
     * 
     * @param value
     *            the value to be resolved
     ...
     * If there are inconsistencies, such as when a macro value refers to a
     * nonexistent macro
     * or when two macros refer to each other, this method will throw the
     * BuildMacroException exception
     * The BuildMacroException will contain the human-readable string describing
     * the inconsistency and the array of the IBuildMacro interfaces that will
     * represent the macros that
     * caused the inconsistency. This information will be used in the UI to notify
     * the user about
     * the macro inconsistencies (see also the "User interface for viewing and
     * editing Build Macros"
