...
public class Serial implements SerialPortEventListener {
	...
	public void dispose() {
		notifyConsumersOfEvent("Disconnect of port " + port.getPortName()
				+ " executed");
		disconnect();
			...
			if (IsConnected() && bytesCount > 0) {
				synchronized (buffer) {
					try {
						...
						System.arraycopy(readBytes, 0, buffer, bufferLast,
								readBytes.length);
						if (monitor == true) {
	...
	public void write(String what, String LineEnd) {
		notifyConsumersOfEvent(System.getProperty("line.separator")
				+ ">>Send to " + PortName + ": \"" + what + "\"<<"
