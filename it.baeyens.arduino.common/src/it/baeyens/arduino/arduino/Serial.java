...
public class Serial implements SerialPortEventListener
	{
				...
				if (port == null)
					{
						try
							...
							{
								Common.log(new Status(IStatus.ERROR, ArduinoConst.CORE_PLUGIN_ID, "Serial port " + PortName + " already in use. Try quiting any programs that may be using it", e));
								return;
							} catch (Exception e)
							{
		...
		public void dispose()
			{
				if ((consumer != null) & (port != null))
					{
						consumer.message("Disconnect of port " + port.getName() + " executed");
					}
				consumer = null;
				disconnect();
			}
								...
								synchronized (buffer)
									{
										...
										if (this.consumer != null)
											{
												this.consumer.message(readString());
											}
									}
							} catch (IOException e)
							{
				...
				synchronized (buffer)
					{
						...
						if (length > outgoing.length)
							{
								Common.log(new Status(IStatus.WARNING, ArduinoConst.CORE_PLUGIN_ID, "readBytesUntil() byte buffer is too small for the " + length + " bytes up to and including char " + interesting,
										null));
								return -1;
							}
						// byte outgoing[] = new byte[length];
		...
		public void write(String what, String LineEnd)
			{
				consumer.message(System.getProperty("line.separator") + ">>Send to " + PortName + ": \"" + what + "\"<<" + System.getProperty("line.separator"));
				write(what.getBytes());
				if (LineEnd.length() > 0)
					{
