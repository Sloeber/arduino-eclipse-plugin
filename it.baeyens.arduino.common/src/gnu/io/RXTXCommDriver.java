...
public class RXTXCommDriver implements CommDriver {
	 ...
	 * 
	 * It could be possible to move this over to RXTXVersion but All we want to do is warn people when first loading the Library.
	 */
	 ...
	 * 
	 * On a Linux system there are about 400 prefixes in deviceDirectory. registerScannedPorts() assigns CandidatePortPrefixes to something less
	 * than 50 prefixes.
	 * 
	 * Trent
	 */
	...
	int p = 0;
	/*
	 * FIX ME quick fix to get COM1-8 on windows working. The Read test is not working properly and its crunch time...
	 * if(osName.toLowerCase().indexOf("windows") != -1 ) { for( i=0;i < CandidateDeviceNames.length;i++ ) { CommPortIdentifier.addPortName(
	...
	if (CandidateDeviceNames != null && ValidPortPrefixes != null) {
		...
		for (p = 0; p < ValidPortPrefixes.length; p++) {
		    /*
		     * this determines: device file Valid ports /dev/ttyR[0-9]* != /dev/ttyS[0-9]* /dev/ttySI[0-9]* != /dev/ttyS[0-9]* /dev/ttyS[0-9]*
		     * == /dev/ttyS[0-9]*
		     * 
    ...
    /*
     * initialize() will be called by the CommPortIdentifier's static initializer. The responsibility of this method is: 1) Ensure that that the
     * hardware is present. 2) Load any required native libraries. 3) Register the port names with the CommPortIdentifier.
     * 
    ...
    /*
     * Register ports specified in the file "gnu.io.rxtx.properties" Key system properties: gnu.io.rxtx.SerialPorts gnu.io.rxtx.ParallelPorts
     * 
    ...
    /*
     * Look for all entries in deviceDirectory, and if they look like they should be serial ports on this OS and they can be opened then register
     * them.
     */
	...
	if (osName.equals("Windows CE")) {
	    ...
	    /*
	     * Solaris uses a few different ways to identify ports. They could be /dev/term/a /dev/term0 /dev/cua/a /dev/cuaa the /dev/???/a appears
	     * to be on more systems.
	     * 
	     * The uucp lock files should not cause problems.
	     */
	    /*
	     * File dev = new File( "/dev/term" ); String deva[] = dev.list(); dev = new File( "/dev/cua" ); String devb[] = dev.list(); String[] temp
	     * = new String[ deva.length + devb.length ]; for(int j =0;j<deva.length;j++) deva[j] = "term/" + deva[j]; for(int j =0;j<devb.length;j++)
	     * devb[j] = "cua/" + devb[j]; System.arraycopy( deva, 0, temp, 0, deva.length ); System.arraycopy( devb, 0, temp, deva.length,
	     * devb.length ); if( debug ) { for( int j = 0; j< temp.length;j++) System.out.println( temp[j] ); } CandidateDeviceNames=temp;
	     */
	    ...
	    /*
	     * dev = new File( "/dev/cua0" ); if( dev.list().length > 0 ); term[l++] = new String( "cua/" );
	     */
	...
	case CommPortIdentifier.PORT_SERIAL:
	    ...
	    /*
	     * There are _many_ possible ports that can be used on Linux. See below in the fake Linux-all-ports case for a list. You may add
	     * additional ports here but be warned that too many will significantly slow down port enumeration. Linux 2.6 has udev support which
	     * should be faster as only ports the kernel finds should be exposed in /dev
	     * 
		...
		CandidatePortPrefixes = Temp;
	    } else if (osName.equals("FreeBSD")) // FIX ME this is probably
						 // wrong
	    {
		...
		String[] Temp = {
			...
			"tty2A", "tty3A", "tty4A", "tty5A", "tty6A", "tty7A", "tty8A", "tty9A", "tty10A", "tty11A", "tty12A", "tty13A", "tty14A",
			"tty15A",
			"tty16A",
			"ttyu1A", // OSR5 USB-serial
				  // ports
			"ttyu2A", "ttyu3A", "ttyu4A", "ttyu5A", "ttyu6A", "ttyu7A", "ttyu8A", "ttyu9A", "ttyu10A", "ttyu11A", "ttyu12A", "ttyu13A",
			"ttyu14A", "ttyu15A", "ttyu16A" };
		CandidatePortPrefixes = Temp;
	...
	case CommPortIdentifier.PORT_PARALLEL:
	    ...
	    if (osName.equals("Linux")
	    /*
	     * || osName.equals("NetBSD") FIX ME || osName.equals("HP-UX") FIX ME || osName.equals("Irix") FIX ME || osName.equals("BeOS") FIX ME ||
     ...
     * @return CommPort getCommPort() will be called by CommPortIdentifier from its openPort() method. PortName is a string that was registered
     *         earlier using the CommPortIdentifier.addPortName() method. getCommPort() returns an object that extends either SerialPort or
     *         ParallelPort.
     */
    @Override
    ...
    public static void Report(String arg) {
