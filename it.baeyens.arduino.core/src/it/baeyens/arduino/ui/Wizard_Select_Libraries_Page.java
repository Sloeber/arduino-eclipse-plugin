...
public class Wizard_Select_Libraries_Page extends WizardResourceImportPage {
	...
	URI ArduinoLibraryURI = pathMan
		.getURIValue(ArduinoConst.WORKSPACE_PATH_VARIABLE_NAME_ARDUINO_LIB);
	URI PrivateLibraryURI = pathMan
		.getURIValue(ArduinoConst.WORKSPACE_PATH_VARIABLE_NAME_PRIVATE_LIB);
	URI HardwareLibrarURI = pathMan
		.getURIValue(ArduinoConst.WORKSPACE_PATH_VARIABLE_NAME_HARDWARE_LIB);
	...
	if (ArduinoLibItem != null) {
	    importLibs(ArduinoConst.WORKSPACE_PATH_VARIABLE_NAME_ARDUINO_LIB,
		    ArduinoLibItem.getItems());
	}
	if (PersonalLibItem != null) {
	    importLibs(ArduinoConst.WORKSPACE_PATH_VARIABLE_NAME_PRIVATE_LIB,
		    PersonalLibItem.getItems());
	}
	if (ArduinoHardwareLibItem != null) {
	    importLibs(ArduinoConst.WORKSPACE_PATH_VARIABLE_NAME_HARDWARE_LIB,
		    ArduinoHardwareLibItem.getItems());
	}
	return true;
    }
	...
	for (int curConfig = 0; curConfig < configurationDescriptions.length; curConfig++) {
		...
		if (AllItems[CurItem].getChecked()) {
		    try {
			ArduinoHelpers.addCodeFolder(mProject, PathVarName,
				AllItems[CurItem].getText(),
				ArduinoConst.WORKSPACE_LIB_FOLDER
					+ AllItems[CurItem].getText(),
				configurationDescriptions[curConfig]);
