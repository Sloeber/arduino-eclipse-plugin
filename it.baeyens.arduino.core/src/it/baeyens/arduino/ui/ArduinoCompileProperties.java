...
public class ArduinoCompileProperties extends AbstractCPropertyTab {
	...
	public void createControls(Composite parent, ICPropertyProvider provider) {
			...
			public void handleEvent(Event e) {
				if (getResDesc().getConfiguration() != null) {
					...
					if (myWarningLevel.getSelection() == true) {
						IEnvironmentVariable var = new EnvironmentVariable(ArduinoConst.ENV_KEY_WARNING_LEVEL, ArduinoConst.ENV_KEY_WARNING_LEVEL_ON);
						contribEnv.addVariable(var, getResDesc().getConfiguration());
					} else {
						IEnvironmentVariable var = new EnvironmentVariable(ArduinoConst.ENV_KEY_WARNING_LEVEL, ArduinoConst.ENV_KEY_WARNING_LEVEL_OFF);
						contribEnv.addVariable(var, getResDesc().getConfiguration());
					}
				}
			}
			...
			public void handleEvent(Event e) {
				if (getResDesc().getConfiguration() != null) {
					...
					if (mySizeCommand.getSelection() == true) {
						IEnvironmentVariable var = new EnvironmentVariable(ArduinoConst.ENV_KEY_SIZE_SWITCH, "${"
		...
		myCCppCommand.setMessage("This command is added to the C and C++ compile command.");
		myCCppCommand.setEnabled(true);
		myCCppCommand.setLayoutData(new GridData(SWT.BEGINNING, SWT.BEGINNING, false, false, 1, 1));
		...
		myCppCommand.setMessage("This command is added to the C++ compile command.");
		myCppCommand.setEnabled(true);
		myCppCommand.setLayoutData(new GridData(SWT.BEGINNING, SWT.BEGINNING, false, false, 1, 1));
		...
		myCCommand.setText("");
		myCCommand.setMessage("This command is added to the C compile command.");
		myCCommand.setEnabled(true);
		myCCommand.setLayoutData(new GridData(SWT.BEGINNING, SWT.BEGINNING, false, false, 1, 1));
	...
	private void setValues(ICConfigurationDescription confDesc) {
		IEnvironmentVariableManager envManager = CCorePlugin.getDefault().getBuildEnvironmentManager();
		IContributedEnvironment contribEnv = envManager.getContributedEnvironment();
		IEnvironmentVariable var = contribEnv.getVariable(ArduinoConst.ENV_KEY_WARNING_LEVEL, confDesc);
		if (var == null)
		...
		var = contribEnv.getVariable(ArduinoConst.ENV_KEY_SIZE_SWITCH, confDesc);
		if (var == null)
