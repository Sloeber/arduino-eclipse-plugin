...
public final class CfgInfoContext {
    ...
    private static CfgInfoContext doCreate(IConfiguration cfg, String[] ids, InfoContext context) {
        ...
        if (toolId != null) {
            tool = (ITool)rcInfo.getToolById(toolId);
            if (tool == null)
                return null;
        }

        if (inTypeId != null) {
            inType = tool.getInputTypeByID(inTypeId);
            if (inType == null)
                return null;
        }
    ...
    private static IBuildObject find(List<IBuildObject> objs, String id) {
        for (IBuildObject obj: objs) {
