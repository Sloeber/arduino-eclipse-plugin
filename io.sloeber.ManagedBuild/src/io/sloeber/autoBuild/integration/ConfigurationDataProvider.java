...
public class ConfigurationDataProvider extends CConfigurationDataProvider {// implements ISettingsChangeListener {
	...
	static BuildConfigurationData writeConfiguration(ICConfigurationDescription cfgDescription,
		...
		// I ùean 2 parameters are provided and one parameter is used to set the
		// description of the other via a
		// a lot of detours
		// if (b != null && b.isManagedBuildOn() && b.getBuildPathAttribute(false) ==
		// null) {
		// String bPath = b.getDefaultBuildPath();
		// b.setBuildPathAttribute(bPath);
		// }
		// // cfg.setConfigurationDescription(des);
		// // ManagedBuildManager.performValueHandlerEvent(cfg,
		...
		if (context.isBaseDataCached()) {// JABA Assume not dirty && !baseCfg.isDirty()
			...
			if (baseCfgDescription instanceof ILanguageSettingsProvidersKeeper) {
				...
				if (defaultIds == null) {
					...
					if (cfg != null) {
						defaultIds = cfg.getDefaultLanguageSettingsProviderIds().toArray(new String[0]);
					}
					if (defaultIds == null) {
...
//        return emptyPrefCfg;
	...
	// BuildLanguageData datas[] = (BuildLanguageData[])
	// event.getRcInfo().getCLanguageDatas();
