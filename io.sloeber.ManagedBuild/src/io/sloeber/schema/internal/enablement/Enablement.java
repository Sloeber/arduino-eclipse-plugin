...
public class Enablement {
	...
	public boolean isEnabled(AutoBuildConfigurationData autoBuildConfData) {
		...
		try {
			result = myExpression.evaluate(new EvaluationContext(null, autoBuildConfData));
	...
	public Enablement(IConfigurationElement element) {
		...
		handlers[0] = new ElementHandler() {
			...
			@Override
			public Expression create(ExpressionConverter converter, IConfigurationElement config) throws CoreException {
				...
				case "checkBuildProperty": //$NON-NLS-1$
					return new BuildPropertyExpression(element);
